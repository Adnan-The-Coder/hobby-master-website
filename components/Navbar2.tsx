...
const Navbar2 = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-colors duration-300 ${scrolled ? 'bg-black' : 'bg-black'} opacity-90`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex-shrink-0">
            <Link href="/" className={`text-3xl hover:text-green-600 font-bold ${scrolled ? 'text-white' : 'text-white'}`}>Hobby Master
            </Link>
          </div>
      ...
      <div className={`md:hidden ${menuOpen ? 'block bg-gray-800' : 'hidden'}`} id="mobile-menu">
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          <Link href="/" className={`text-xl text-white hover:text-green-600 block px-3 py-2 rounded-md text-base font-medium`} onClick={() => { toggleMenu();  }}>
            Home
          </Link>
          <Link href="/Blog" className={`text-xl ${scrolled ? 'text-black' : 'text-white'} hover:text-green-600 px-3 py-2 rounded-md text-sm font-medium`} >
                Blog
              </Link>
          <Link href="/About" className={`text-xl text-white hover:text-green-600 block px-3 py-2 rounded-md text-base font-medium`} onClick={() => { toggleMenu(); }}>
            About
          </Link>
